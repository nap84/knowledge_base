...
<!-- The first half of the sentence sounds grammatically not correct (but I probably err) -->
...
<!-- instead of narrowing it down to an XML format, maybe just "machine readable" format. I think not all of the metadata are stored as XML?! -->
...
<!-- In the following sentence you address the reader directly?!
suggestion: ... data sets, persistent identifiers, such as e.g. DOIs, are suggested which resolved by ... -->

To guarantee access to data sets, use identifiers such as DOIs that are resolved by standard methods. Common protocols include http(s) or (s)ftp.

#### A1.1 the protocol is open, free, and universally implementable

Repositories should only use protocols that allow any computer to access at least the metadata. Not only does this refer to standard communication protocols, as stated in A1, they must also be free <!-- free of what? freely available? --> and open-sourced. Therefore, proprietary or non-standard protocols should be avoided.

#### A1.2 the protocol allows for an authentication and authorization procedure, where necessary

Where necessary, machine-readable protocols that let the user know that action needs to be taken (such as a login) to access data. <!-- is the previous sentence complete? -->
...
Storing metadata is cheaper and simpler than storing and maintaining full datasets. Furthermore, the dataset itself may still be available; however, the file format may be outdated, thus limiting access to the full data. <!-- The previous sentence is difficult to understand. Maybe you can write it more in detail: The separation of the data and the describing metadata need to be done on a file level. This is due since ...1) cost 2) outdating 3) ?. ... -->
Therefore, the metadata should be stored in a separate file, which should be archived forever. A repository should clearly state a contingency plan for metadata storage should the service no longer exist, such as migrating to another repository service provider.

<!-- and to make sure the integrity of the persistent identifier are garanteed. -->

## Interoperable

Data need to be integrated with and/or compared to other datasets, while computers must be able to interpret and exchange the information.

<!-- To me those are not the (main) reasons that data need to be interoperable. One can add that data needs to be interoperable to be able to use it for other purposes.... hm, ok - this is closely related to "Re-Use"?! interoperability is the necessity for re-use.  -->

Ideally, they are compatible with standard applications and can thus be integrated into (automated) processing and analysis workflows.

### I1. (meta)data use a formal, accessible, shared, and broadly applicable language for knowledge representation

Machines need to be able to understand how to exchange and interpret information. Similar to humans, an uniform and standard language aids in this understanding. In chemistry, a great typical example of such an information exchange standard is the [crystallographic information (CIF)](https://doi.org/10.1107/97809553602060000107). This standard also adheres to the aspects described in I2 and R1.3 below.
